...
- Public name and mod ID are exactly `container-search`; Java package is `dev.smpb.containersearch`.
...
- `src/main/resources/assets/container-search/lang/en_us.json`: visible copy.
- `README.md`: installation, supported scope, controls, and limitations.
- `src/main/java/dev/smpb/containersearch/ContainerSearchMod.java`: environment-neutral constants only.

Pure modules:

- `src/main/java/dev/smpb/containersearch/model/*`: immutable item, slot, source, observation, and world records.
- `src/main/java/dev/smpb/containersearch/index/*`: `ContainerIndex` interface and in-memory implementation.
- `src/main/java/dev/smpb/containersearch/store/*`: schema DTOs, codec, and atomic world store.
- `src/main/java/dev/smpb/containersearch/retrieval/*`: one-source transaction state machine.
- `src/main/java/dev/smpb/containersearch/recipe/*`: recipe graph and recursive planner.
- `src/main/java/dev/smpb/containersearch/batch/*`: multi-source material-retrieval coordinator.

Minecraft adapters and UI:

- `src/client/java/dev/smpb/containersearch/ContainerSearchClient.java`: composition root and lifecycle.
- `src/client/java/dev/smpb/containersearch/minecraft/*`: world, container, stack, recipe, reachability, and transfer adapters.
- `src/client/java/dev/smpb/containersearch/ui/ContainerSearchOverlay.java`: search field and slot dimming.
- `src/client/java/dev/smpb/containersearch/ui/CatalogScreen.java`: global split catalog and craft-request UI.
- `src/client/java/dev/smpb/containersearch/ui/CatalogController.java`: immutable UI state and commands.
...
- Create: `src/main/java/dev/smpb/containersearch/ContainerSearchMod.java`
- Create: `src/client/java/dev/smpb/containersearch/ContainerSearchClient.java`
- Test: `src/test/java/dev/smpb/containersearch/ScaffoldTest.java`

**Interfaces:**
- Produces `ContainerSearchClient implements ClientModInitializer` as the only Fabric entrypoint.
...
archives_base_name=container-search
junit_version=6.1.0
```
...
public final class ContainerSearchMod {
    public static final String MOD_ID = "container-search";
    private ContainerSearchMod() { }
}
```
...
Commit: `chore: scaffold container-search fabric mod`

### Task 2: Define item, source, and observation values

**Files:**
- Create: `src/main/java/dev/smpb/containersearch/model/{BlockPosition,ContainerKind,StackKey,StackSnapshot,SlotSnapshot,SourceKey,ContainerObservation}.java`
- Test: `src/test/java/dev/smpb/containersearch/model/ModelTest.java`
...
public record SourceKey(String dimension, ContainerKind kind, List<BlockPosition> positions) {
    ...
    public static SourceKey enderInventory() { return new SourceKey("container-search:all_dimensions", ContainerKind.ENDER_CHEST, List.of()); }
}
...
**Files:**
- Create: `src/main/java/dev/smpb/containersearch/index/{ContainerIndex,InMemoryContainerIndex,IndexSnapshot,ItemResult,SourceResult,SearchQuery}.java`
- Test: `src/test/java/dev/smpb/containersearch/index/InMemoryContainerIndexTest.java`
...
**Files:**
- Create: `src/main/java/dev/smpb/containersearch/store/{WorldKey,WorldStore,JsonWorldStore,StoreDocument,StoreResult,AtomicFileWriter,SavedCraftRequest}.java`
- Test: `src/test/java/dev/smpb/containersearch/store/JsonWorldStoreTest.java`
...
**Files:**
- Create: `src/client/java/dev/smpb/containersearch/minecraft/{MinecraftWorldAdapter,MinecraftContainerAdapter,MinecraftStackAdapter,PendingInteraction}.java`
- Modify: `src/client/java/dev/smpb/containersearch/ContainerSearchClient.java`
- Test: `src/test/java/dev/smpb/containersearch/minecraft/ContainerShapeTest.java`
...
**Files:**
- Create: `src/client/java/dev/smpb/containersearch/ui/{ContainerSearchOverlay,CatalogScreen,CatalogController,CatalogViewModel,ReachabilityView}.java`
- Create: `src/client/java/dev/smpb/containersearch/minecraft/MinecraftReachabilityAdapter.java`
- Modify: `ContainerSearchClient.java`, `en_us.json`
- Test: `src/test/java/dev/smpb/containersearch/ui/CatalogControllerTest.java`
...
- [ ] **Step 2: Implement `ContainerSearchOverlay` with Fabric screen events: an `EditBox`, `Ctrl+F`, first-Escape clear, and draw-time dimming of only nonmatching storage slots. Do not alter slot coordinates or clicks.**
...
**Files:**
- Create: `src/main/java/dev/smpb/containersearch/retrieval/{RetrievalRequest,QuantityRequest,RetrievalState,RetrievalResult,RetrievalPort,RetrievalTransaction,Validation,LiveContainer,TransferProgress,CancelReason}.java`
- Test: `src/test/java/dev/smpb/containersearch/retrieval/RetrievalTransactionTest.java`
...
**Files:**
- Create: `src/client/java/dev/smpb/containersearch/minecraft/MinecraftRetrievalPort.java`
- Modify: `CatalogController.java`, `CatalogScreen.java`, `ContainerSearchClient.java`
...
Run: `./gradlew test build` and `rg -n 'ERROR|Exception|container-search' run/logs/latest.log`
Expected: build passes; no container-search error/exception.
...
**Files:**
- Create: `src/main/java/dev/smpb/containersearch/recipe/{IngredientChoice,RecipeNode,RecipeGraph,Stock,StockLocation,CraftRequest,CraftStage,RecipePlan,RecipePlanner}.java`
- Test: `src/test/java/dev/smpb/containersearch/recipe/RecipePlannerTest.java`
...
**Files:**
- Create: `src/client/java/dev/smpb/containersearch/minecraft/MinecraftRecipeAdapter.java`
- Modify: `CatalogController.java`, `CatalogScreen.java`, `ContainerSearchClient.java`, `en_us.json`
- Test: `src/test/java/dev/smpb/containersearch/recipe/RecipeAdapterContractTest.java`
...
**Files:**
- Create: `src/main/java/dev/smpb/containersearch/batch/{MaterialAllocation,BatchState,BatchResult,RetrievalBatch,InventoryCapacity,ChildRetrievalFactory}.java`
- Modify: `CatalogController.java`, `CatalogScreen.java`
- Test: `src/test/java/dev/smpb/containersearch/batch/RetrievalBatchTest.java`
...
Expected: `BUILD SUCCESSFUL`, all tests green, and distributable `container-search-0.1.0.jar`.

Run: `rg -n 'ERROR|Exception' run/logs/latest.log`
Expected: no container-search-caused error or exception.
...
Commit: `docs: complete container-search release verification`
