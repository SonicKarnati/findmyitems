...
import dev.smpb.containersearch.craft.CraftingPlanner;
...
import dev.smpb.containersearch.retrieval.GhostOpen;
...
public final class CatalogScreen extends Screen {
    ...
    protected void init() {
        ...
        amountField = new EditBox(font, left + searchWidth + GAP, SEARCH_Y, AMOUNT_WIDTH, WIDGET_HEIGHT,
                Component.translatable("screen.container-search.amount"));
    ...
    private void refreshChrome() {
        ...
        searchField.setHint(Component.translatable(switch (view) {
            case ITEMS -> "screen.container-search.hint.items";
            case CONTAINERS -> "screen.container-search.hint.containers";
            case CRAFTING -> "screen.container-search.hint.crafting";
        }));
    }

    private Component tabLabel(View candidate) {
        return Component.translatable(switch (candidate) {
            case ITEMS -> "screen.container-search.view.items";
            case CONTAINERS -> "screen.container-search.view.containers";
            case CRAFTING -> "screen.container-search.view.crafting";
        });
    }
    ...
    private Component layoutLabel() {
        return Component.translatable(layout == Layout.LIST
                ? "screen.container-search.layout.grid"
                : "screen.container-search.layout.list");
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        var footer = switch (view) {
            case ITEMS -> Component.translatable(resultCount == 1
                    ? "screen.container-search.footer.items.one"
                    : "screen.container-search.footer.items", resultCount, amount);
            case CONTAINERS -> Component.translatable("screen.container-search.footer.containers", resultCount);
            case CRAFTING -> currentQuery.isBlank()
                    ? Component.translatable("screen.container-search.footer.craft_index", resultCount)
                    : Component.translatable("screen.container-search.footer.crafting", amount);
        };
    ...
    private List<Row> itemRows() {
        ...
        if (results.isEmpty()) {
            status = currentQuery.isEmpty()
                    ? Component.translatable("screen.container-search.empty").getString()
                    : Component.translatable("screen.container-search.no_results", currentQuery).getString();
            return List.of();
        }
    ...
    private List<Row> containerRows() {
        ...
        if (cards.isEmpty()) {
            status = Component.translatable("screen.container-search.no_containers").getString();
            return List.of();
        }
        return layout == Layout.LIST
    ...
    private List<Row> craftingRows() {
        ...
        if (server == null || level == null) {
            status = Component.translatable("screen.container-search.craft.singleplayer_only").getString();
            return List.of();
        }

        var target = resolveItem(currentQuery);
        if (target == null) {
            status = Component.translatable("screen.container-search.craft.unknown_item", currentQuery).getString();
            return List.of();
        }
    ...
    private record ContainerCard(SourceKey key, ContainerKind kind, int itemCount, String contents, double distanceSqr) {
        ...
        String position() {
            if (key.positions().isEmpty()) return Component.translatable("screen.container-search.anywhere").getString();
    ...
    private final class ItemRow extends Row {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float a) {
            ...
            var outOfReach = Component.translatable("screen.container-search.tooltip.out_of_reach");
            ...
            actionButton(graphics, locateX, buttonY, Items.ENDER_EYE, mouseX, mouseY, nearest != null, nearest != null
                    ? Component.translatable("screen.container-search.locate", sourceLabel(nearest))
                    : Component.translatable("screen.container-search.tooltip.nowhere"));
            ...
            actionButton(graphics, depositX, buttonY, Items.CHEST, mouseX, mouseY, canDeposit,
                    !reachable ? outOfReach
                            : held == 0 ? Component.translatable("screen.container-search.deposit.none")
                            : Component.translatable("screen.container-search.deposit", Math.min(amount, held)));
            if (canDeposit) {
            ...
            actionButton(graphics, takeX, buttonY, Items.HOPPER, mouseX, mouseY, reachable, reachable
                    ? Component.translatable("screen.container-search.take", amount)
                    : outOfReach);
        ...
        private String subtitle(SourceResult nearest) {
            ...
            var where = Component.translatable(containers == 1
                    ? "screen.container-search.in_container"
                    : "screen.container-search.in_containers", containers).getString();
    ...
    private final class ItemGridRow extends Row {
        ...
        @Override
        public Component getNarration() {
            return Component.translatable("screen.container-search.view.items");
        }
    }
    ...
    private final class ContainerRow extends Row {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float a) {
            ...
            actionButton(graphics, locateX, buttonY, Items.ENDER_EYE, mouseX, mouseY, locatable, locatable
                    ? Component.translatable("screen.container-search.locate", kindLabel(card.kind()))
                    : Component.translatable("screen.container-search.tooltip.nowhere"));
        ...
        private String summary() {
            if (card.itemCount() == 0) {
                return Component.translatable("screen.container-search.container.empty").getString();
            }
            return Component.translatable("screen.container-search.container.holds", card.itemCount()).getString()
            ...
            for (var card : cards) {
                ...
                if (mouseX >= x && mouseX < x + SLOT_SIZE && mouseY >= y && mouseY < y + SLOT_SIZE) {
                    ...
                    graphics.setComponentTooltipForNextFrame(font, List.of(
                            ...
                            Component.translatable("screen.container-search.container.holds", card.itemCount())
                    ), mouseX, mouseY);
                }
    ...
    private final class MaterialRow extends Row {
        ...
        public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float a) {
            ...
            actionButton(graphics, locateX, buttonY, Items.ENDER_EYE, mouseX, mouseY, found != null, found != null
                    ? Component.translatable("screen.container-search.locate", stack.getHoverName().getString())
                    : Component.translatable("screen.container-search.tooltip.nowhere"));
            if (found != null) {
        ...
        private String status() {
            if (material.satisfied()) {
                return Component.translatable("screen.container-search.craft.have", material.available()).getString();
            }
            var key = material.isDeadEnd()
                    ? "screen.container-search.craft.short"
                    : "screen.container-search.craft.craft";
