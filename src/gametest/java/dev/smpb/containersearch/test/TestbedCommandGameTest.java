...
/**
 * Checks that {@code /cstest build} really produces a usable testbed and that {@code /cstest clear}
...
public final class TestbedCommandGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        try (var singleplayer = context.worldBuilder().create()) {
            ...
            if (containers[0] < 12) {
                throw new AssertionError("expected at least 12 containers from /cstest build, found " + containers[0]);
            }
            if (containers[1] < 30) {
            ...
            var indexed = context.computeOnClient(mc -> ContainerSearchClient.index().snapshot().containers().size());
            if (indexed == 0) {
            ...
            asPlayer(server, "cstest clear");
            context.waitTicks(10);

            var after = solidBlocksAround(server);
            if (after != before) {
                throw new AssertionError("/cstest clear should restore the world exactly: "
