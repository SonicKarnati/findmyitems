...
public final class ViewportLayout {
    ...
    public static Layout layout(int top, int bottom, int rowHeight, int rowCount, double scroll, int overscan) {
        return layout(0, Integer.MAX_VALUE, top, bottom, rowHeight, rowCount, scroll, overscan);
    }

    public static Layout layout(int left, int right, int top, int bottom, int rowHeight, int rowCount,
                                double scroll, int overscan) {
        var height = Math.max(0, bottom - top);
        if (rowHeight <= 0 || rowCount <= 0 || height == 0) {
            return new Layout(left, right, top, bottom, rowHeight, 0, 0, 0, -1, List.of());
        }
    ...
    public record Layout(int left, int right, int top, int bottom, int rowHeight, int rowCount, double scroll,
        ...
        public OptionalInt hitTest(double x, double y) {
            if (!Double.isFinite(x) || x < left || x >= right || !Double.isFinite(y) || y < top || y >= bottom
